...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const contentType = interaction.options.getString('content_type') || 'application/json';
    ...
    if (!allowsBody && bodyStr) {
      await interaction.reply({ content: '❌ 이 메서드는 요청 본문을 사용하지 않습니다. (GET/DELETE 등)', ephemeral: true });
      return;
    }

    if (allowsBody && bodyStr && /^application\/json/i.test(contentType)) {
      try { JSON.parse(bodyStr); } catch {
        await interaction.reply({ content: '❌ body가 유효한 JSON이 아닙니다.', ephemeral: true });
        return;
      }
    }
    ...
    const { res, elapsed, errInfo } = await fetchWithTimeout(target.toString(), init, 10000);
